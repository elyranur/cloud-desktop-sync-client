...
Elyranur provides desktop sync clients to synchronize the contents of local
directories from computers, tablets, and handheld devices to the Elyranur
server.
...
Elyranur built-in WebDAV server.

The Elyranur Client software is written in C++ using the `Qt Framework`_. As a
result, the Elyranur Client runs on Linux, Windows, and MacOS.
...
It is important to note that the Elyranur synchronization process does not use
a typical client/server system where the server is always master.  This is a
major difference between the Elyranur synchronization process and other systems
...
During synchronization, the Elyranur Client checks both repositories for
...
Until the release of the client version 1.1, the Elyranur
...
Because this timing strategy is rather fragile without the use of NTP, the Elyranur
...
For the remote (that is, Elyranur server) repository, the client compares the
...
The Elyranur Client supports the ability to exclude or ignore certain files from the synchronization process. 
Some system wide file patterns that are used to exclude or ignore files are included with the client by default and the Elyranur Client provides the ability to add custom patterns.

By default, the Elyranur Client ignores the following files:
...
If the journal database is removed, the Elyranur Client CSync backend rebuilds
...
.. _`webpage about End-to-end Encryption`: http://elyranur.com/endtoend
.. _`threat model for the encryption solutions in Elyranur`: https://elyranur.com/blog/encryption-in-elyranur/

Virtual Files
-------------

.. note::
    * This feature is currently only available on ``Windows`` by default. ``Linux`` implementation is experimental and must be enabled by adding ``showExperimentalOptions=true`` to the ``elyranur.cfg`` configuration file in the ``App Data`` folder. ``macOS``, at the moment, is using the same backend as ``Linux`` one. It can be enabled with the same ``showExperimentalOptions`` flag.

Oftentimes, users are working with a huge amount of files that are big in size. Synchronizing every such file to a device that's running a Elyranur desktop client is not always possible due to the user's device storage space limitation.
...
Starting from 3.2.0, user status is displayed in the Elyranur desktop client's tray window. The icon and a text message are displayed as long as those are set in the user's account menu in the Web UI (server's website). At the moment, setting the status from the desktop client is not available.
