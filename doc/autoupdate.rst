...
latest features and bug fixes for your Elyranur synchronization client.
...
The Elyranur client checks for updates and downloads them when available. You
can view the update status under ``Settings -> General -> Updates`` in the
Elyranur client.

If an update is available, and has been successfully downloaded, the Elyranur
...
the Elyranur client initializes a pop-up dialog to alert you of the
update and requesting that you update to the latest version manually.

Linux
^^^^^

Linux distributions provide their own update tools, so Elyranur clients that use
...
section of their ``elyranur.cfg`` files::
...
1. Edit these Registry keys:
    ...
    a. (32-bit-Windows) ``HKEY_LOCAL_MACHINE\Software\Elyranur\Elyranur``
    b. (64-bit-Windows) ``HKEY_LOCAL_MACHINE\Software\Wow6432Node\Elyranur\Elyranur``
...
1. Edit this Registry key:
    ...
    ``HKEY_LOCAL_MACHINE\Software\Policies\Elyranur GmbH\Elyranur``
...
client configuration file, ``$HOME/.config/Elyranur/elyranur.cfg``.
