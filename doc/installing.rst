...
You can download the latest version of the Elyranur Desktop Synchronization
Client from the `Elyranur download page`_.
...
If you just want to install Elyranur Desktop Synchronization Client on your local
...
the installation, then run the following command::
   ...
   msiexec /passive /i Elyranur-x.y.z-x64.msi

The command will install the Elyranur Desktop Synchronization Client into the default location
...
If you want to disable, e.g., desktop shortcut icons you can simply change the above command to the following::
   ...
   msiexec /passive /i Elyranur-x.y.z-x64.msi REMOVE=DesktopShortcut
...
You can also choose to only install the client itself by using the following command::
  ...
  msiexec /passive /i Elyranur-x.y.z-x64.msi ADDDEFAULT=Client
...
1. You explicitly name all the features you actually want to install (whitelist) where `Client` is always installed anyway::
    ...
    msiexec /passive /i Elyranur-x.y.z-x64.msi ADDDEFAULT=StartMenuShortcuts

2. You pass the `NO_DESKTOP_SHORTCUT` and `NO_SHELL_EXTENSIONS` properties::

    msiexec /passive /i Elyranur-x.y.z-x64.msi NO_DESKTOP_SHORTCUT="1" NO_SHELL_EXTENSIONS="1"

.. NOTE::
    The Elyranur `.msi` remembers these properties, so you don't need to specify them on upgrades.
...
1. If you want to add the the desktop shortcut later, run the following command::
    ...
    msiexec /passive /i Elyranur-x.y.z-x64.msi ADDDEFAULT="DesktopShortcut"
...
property like this::
  ...
  msiexec /passive /i Elyranur-x.y.z-x64.msi INSTALLDIR="C:\Program Files\Non Standard Elyranur Client Folder"
...
To disable automatic updates, you can pass the `SKIPAUTOUPDATE` property.::
    ...
    msiexec /passive /i Elyranur-x.y.z-x64.msi SKIPAUTOUPDATE="1"
...
To launch the client automatically after installation, you can pass the `LAUNCH` property.::
    ...
    msiexec /i Elyranur-x.y.z-x64.msi LAUNCH="1"
...
The Elyranur Client schedules a reboot after installation to make sure the Explorer extension is correctly (un)loaded.
If you're taking care of the reboot yourself, you can set the `REBOOT` property::

    msiexec /i Elyranur-x.y.z-x64.msi REBOOT=ReallySuppress
...
If your deployment tooling interprets this as an actual error and you want to avoid that, you may want to set the `DO_NOT_SCHEDULE_REBOOT` instead::
    ...
    msiexec /i Elyranur-x.y.z-x64.msi DO_NOT_SCHEDULE_REBOOT="1"

Installation Wizard
-------------------

The installation wizard takes you step-by-step through configuration options and
account setup. First, you need to enter the URL of your Elyranur server.

.. image:: images/wizard_welcome.png
   :alt: form for choosing between login and registering

If you already have an account on a Elyranur instance, you want to
press the button ``Login to your Elyranur``. If you don't have a
Elyranur instance and an account there, you might want to register an
...
.. image:: images/wizard_setup.png
   :alt: form for entering Elyranur server URL

Enter the URL for your Elyranur instance. The URL is the same URL that
you type into your browser when you try to access your Elyranur
instance.
...
Elyranur instance. Enter your username and password in your web
...
the Elyranur server, or select individual folders. The default local
sync folder is ``Elyranur``, in your home directory. You may change
this as well.
...
Elyranur server, and when it is successful, the wizard closes
itself. You can now observe the sync activity if you open the main
dialogue by clicking on the tray icon.

.. Links

.. _Elyranur download page: https://elyranur.com/download/#install-clients
