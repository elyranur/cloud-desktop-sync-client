...
Q_LOGGING_CATEGORY(lcEtagJob, "elyranur.sync.networkjob.etag", QtInfoMsg)
Q_LOGGING_CATEGORY(lcLsColJob, "elyranur.sync.networkjob.lscol", QtInfoMsg)
Q_LOGGING_CATEGORY(lcCheckServerJob, "elyranur.sync.networkjob.checkserver", QtInfoMsg)
Q_LOGGING_CATEGORY(lcPropfindJob, "elyranur.sync.networkjob.propfind", QtInfoMsg)
Q_LOGGING_CATEGORY(lcAvatarJob, "elyranur.sync.networkjob.avatar", QtInfoMsg)
Q_LOGGING_CATEGORY(lcMkColJob, "elyranur.sync.networkjob.mkcol", QtInfoMsg)
Q_LOGGING_CATEGORY(lcProppatchJob, "elyranur.sync.networkjob.proppatch", QtInfoMsg)
Q_LOGGING_CATEGORY(lcJsonApiJob, "elyranur.sync.networkjob.jsonapi", QtInfoMsg)
Q_LOGGING_CATEGORY(lcDetermineAuthTypeJob, "elyranur.sync.networkjob.determineauthtype", QtInfoMsg)
...
namespace {
    ...
    const char elyranurDirC[] = "elyranur/";
}
...
bool CheckServerJob::finished()
{
    ...
    if ((reply()->error() == QNetworkReply::ContentNotFoundError) && (!_subdirFallback)) {
        ...
        setPath(QLatin1String(elyranurDirC) + QLatin1String(statusphpC));
        start();
...
bool JsonApiJob::finished()
{
    ...
    const auto desktopNotificationsAllowed = reply()->rawHeader(QByteArray("X-Elyranur-User-Status"));
