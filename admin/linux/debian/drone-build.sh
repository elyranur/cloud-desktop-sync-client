...
PPA=ppa:elyranur-devs/client
PPA_ALPHA=ppa:elyranur-devs/client-alpha
PPA_BETA=ppa:elyranur-devs/client-beta

OBS_PROJECT=home:ivaradi
OBS_PROJECT_ALPHA=home:ivaradi:alpha
OBS_PROJECT_BETA=home:ivaradi:beta
OBS_PACKAGE=elyranur-desktop
...
if test "$kind" = "alpha"; then
    repo=elyranur-devs/client-alpha
elif test "$kind" = "beta"; then
    repo=elyranur-devs/client-beta
else
    repo=elyranur-devs/client
fi

origsourceopt=""

cp -a ${DRONE_WORKSPACE} elyranur-desktop_${basever}-${revdate}
tar cjf elyranur-desktop_${basever}-${revdate}.orig.tar.bz2 --exclude .git --exclude binary elyranur-desktop_${basever}-${revdate}
...
for distribution in ${UBUNTU_DISTRIBUTIONS} ${DEBIAN_DISTRIBUTIONS}; do
    ...
    fullver=`head -1 debian/changelog | sed "s:elyranur-desktop (\([^)]*\)).*:\1:"`

    EDITOR=true dpkg-source --commit . local-changes

    dpkg-source --build .
    dpkg-genchanges -S -sa > "../elyranur-desktop_${fullver}_source.changes"
...
if test "${pull_request}" = "master"; then
    ...
    if test -f ~/.has_ppa_keys; then
        for distribution in ${UBUNTU_DISTRIBUTIONS}; do
            changes=$(ls -1 elyranur-desktop_*~${distribution}1_source.changes)
        ...
        for distribution in ${DEBIAN_DISTRIBUTIONS}; do
            ...
            cp ../elyranur-desktop*.orig.tar.* ${OBS_SUBDIR}/
            cp ../elyranur-desktop_*[0-9.][0-9]${pkgvertag}.dsc ${OBS_SUBDIR}/
            cp ../elyranur-desktop_*[0-9.][0-9]${pkgvertag}.debian.tar* ${OBS_SUBDIR}/
            cp ../elyranur-desktop_*[0-9.][0-9]${pkgvertag}_source.changes ${OBS_SUBDIR}/
            osc add ${OBS_SUBDIR}/*
