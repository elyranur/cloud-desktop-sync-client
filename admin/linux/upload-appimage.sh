...
export DESKTOP_API_BASE_URL=https://api.github.com/repos/elyranur/desktop

# PR / master
export TAG_NAME=${PR:=master}
export RELEASE_BODY=https://github.com/elyranur/desktop

if [ $TAG_NAME != "master" ]; then
    TAG_NAME="PR-$TAG_NAME"
    RELEASE_BODY="elyranur/desktop#$PR"
fi

cd /build

# AppImage
export APPIMAGE=$(readlink -f ./Elyranur*.AppImage)
...
if [ $TAG_NAME != "master" ]; then
    ...
    curl 2>/dev/null -u $GIT_USERNAME:$GIT_TOKEN -X POST $DESKTOP_API_BASE_URL/issues/$PR/comments -d "{ \"body\" : \"AppImage file: [$BASENAME]($browserDownloadUrl) <br/><br/>To test this change/fix you can simply download above AppImage file and test it. <br/><br/>Please make sure to quit your existing Elyranur app and backup your data. \" }"
fi
